...
<?php $menu ='relatorios_bugs'; require ROOTDIR . '/header/header.php';?> 
...
<div class="container-fluid">
    <div class="container">
        <div class="row">
            <h2><center>Relatório de Bugs</center></h2>
...
// Graficos sobre os Bugs

graphicsBugs();



function graphicsBugs(){

$('.back').fadeIn();
 setTimeout(function(){ 
dataValue='&action=1';
  $.ajax({ url:'relatorio_bugs/action_relatorio_bugs.php',
    data:dataValue,
    type:'POST', 
    cache:false,
    success: function(data){
      ...
      if (arr == null || arr.length < 1)
      {
        ...
        var myChart = new Chart(ctx, {
            type: 'bar',
            data: {
                //labels: ["Pendente", "Analise", "Teste", "Concluido"],
                labels: estado,
                datasets: [{
                    label: '# de Bugs',
                    data: total,
...
$("#data_bug").datetimepicker({ ignoreReadonly: true, format: 'DD/MM/YYYY', 
...
function confirmDeleteBugs(id){
table = $('#example').DataTable();
bootbox.dialog({
  message: "Tem a certeza que pretende apagar o Bug com o numero: #<strong>"+id+"</strong> ?",
  ...
  buttons: {
    ...
    danger: {
     ...
     $.ajax({
      type: "POST",
      cache:false,
      url: "relatorio_bugs/action_relatorio_bugs.php",
      data: dataValue,
      success: function(data){
        if(data == 2){
          table.row("#"+id).remove().draw();
          $('.debug-url').html('O Bug com o numero: #<strong>'+id+'</strong> foi apagado com sucesso.');
          ...
          $('.debug-url').html('O Bug com o numero: #<strong>'+id+'</strong> não foi apagado.');
          $("#mensagem_ko").trigger('click');
      ...
      error:function(data){
        $('.debug-url').html('O Bug com o numero: #<'<strong>+id+'</strong> não foi apagado, p.f. verifique a ligação Wi-Fi.');
        $("#mensagem_ko").trigger('click');
      }
   });
...
{
     if ($("#estado_edit_bug").val() == 'Concluido')
        {
            $( "#data_bug_edit" ).prop('disabled',true);
            $( "input[name='data_bug_edit']" ).prop('disabled',true);
            $( "#descricao_bug_edit" ).prop('disabled',false);
            $( "#link_bug_edit" ).prop('disabled',true);
        }
        if ($("#estado_edit_bug").val() == 'Analise')
        {
            $( "#data_bug_edit" ).prop('disabled',true);
            $( "input[name='data_bug_edit']" ).prop('disabled',true);
        ...
        if ($("#estado_edit_bug").val() == 'Teste')
        {
            $( "#data_bug_edit" ).prop('disabled',false);
            $( "input[name='data_bug_edit']" ).prop('disabled',false);
            $( "#obs_suporte_edit" ).prop('disabled',false);
            $("#estado_edit").prop('disabled',false);
            $( "#descricao_bug_edit" ).prop('disabled',false);
        }
        if ($("#estado_edit_bug").val() == 'Pendente')
        {
            $( "#data_bug_edit" ).prop('disabled',false);
            $( "input[name='data_bug_edit']" ).prop('disabled',false);
            $( "#obs_suporte_edit" ).prop('disabled',false);
...
function editBugsTable(arr)
	{
		var clientesModal = $('#edit_bug');
		
		
		
		
        //Data do Cliente
        $("#data_bug_edit").val(moment(arr.data_ins*1000).format("DD/MM/YYYY"));

        
        // Empresa
        $("#empresa_edit_bug").val(arr.cliente);
        
        //NIF
        $("#nif_bug_edit").val(arr.nif);
        
        
        //Link
         $("#link_bug_edit").val(arr.link);
        ...
        $("#descricao_bug_edit").val(arr.descricao);
        ...
        //Estado
        $("#estado_edit_bug").html('<option value="'+arr.estado+'">'+arr.estado+'</option><option value="Pendente">Pendente</option><option value="Analise">Analise</option><option value="Teste">Teste</option><option value="Concluido">Concluido</option>');
        
        
        // Tipo de Prioridade
        $("#tipo_prioridade_bugs_edit").html('<option value="'+arr.tipo_priodade+'">'+arr.tipo_priodade+'</option><option value="Alta">Alta</option><option value="Média">Média</option><option value="Baixa">Baixa</option>');
	...
	function saveIt(vl){
        ...
        //Descricao
        desc_valor = $("#descricao_bug_edit").val().replace(temp, '');
		...
		dataValue="action=3&id="+vl+"&data_ins="+$('#data_bug_edit').val()+"&link="+$('#link_bug_edit').val()+"&descricao="+total+"&estado="+$("#estado_edit_bug").val()+"&username="+$("#user_name").val()+"&duracao="+$("#duracao_edit").val()+'&tipo_prioridade_bugs='+$("#tipo_prioridade_bugs_edit").val();
...
//console.log(data);
    ...
    myDate=$('#data_bug_edit').val().split("/");
    ...
    data['data_ins'] = x;
        ...
        // Empresa
        data['cliente'] = $("#empresa_edit_bug").val();
        
        //NIF
        data['nif'] = $("#nif_bug_edit").val();
        
        
        //Link
        data['link'] = $("#link_bug_edit").val();
        ...
        //Estado
        data['estado'] = $("#estado_edit_bug").val();
        ...
        data['duracao'] = $("#duracao_edit").val();
        
        data['tipo_priodade'] = $("#tipo_prioridade_bugs_edit").val();
  ...
  table = $('#example').DataTable( {
    ...
    "ajax": 
      {
        "url" : "action_datatables_relatorio_bugs.php",
        "type": "GET"
      },
      columns: [
      ...
      select: 
        ...
        buttons: [
          ...
          },
          {
            ...
            {
              config.filename = 'bugs_relatorios';
              config.title = 'Bugs de Relatorios de Assistencia Técnica';
          ...
          {  
           ...
           },
           {
            ...
            action:function(e,dt,button,config){
              config.filename = 'bugs_relatorios';
              config.title = 'Bugs de Gestão Relatorios de Assistencia Técnica';
