
<?php require ROOTDIR . '/header/head.php' ?>




    <title>Agenda de Gestão de Agendas</title>
</head>



<link href="css/bootstrap.min.css" rel="stylesheet" type="text/css"/> 
  <link href="css/select.dataTables.css" rel="stylesheet" type="text/css"/>
  <link href="css/buttons.dataTables.css" rel="stylesheet" type="text/css"/>
  <link href="css/dataTables.bootstrap.min.css" rel="stylesheet" type="text/css"/>  
  <link href="css/mytables.css" rel="stylesheet" type="text/css"/>
  <link href="css/personalized.css" rel="stylesheet" type="text/css"/> 


<?php $menu ='home'; require ROOTDIR . '/header/header.php';?>


<?php require 'modals.php' ?>





<div style="margin-top:0px; ">




<div class="container-fluid" style="padding: 0px; margin-top: -21px;">
    
    


<!-- Preenchimeto das Datatables por Data -->


<div id="example_container2">
<div class="panel panel-default my_black" style="background-color: #414042!important; padding-bottom: 20px;">
 <div class="container" style="padding-top: 20px;">
    <div class="row">
        
        <div class="search_results">
          <div class="form-group">
            <h3 style="text-align:center; font-size:21px;">
              <span class="label label-default" id="nr_resultados" style="padding: 10px;">
              </span>
            </h3>
          </div>
        </div>
        
        <!-- Data de Pesquisa -->
        <form id="search_data">
        <div class="col-md-3 col-sm-6 col-xs-12">
            <label>Pesquisar Data:</label>
          <div class='input-group date' id='data_pes' >
                              
                              <input type='text' readonly class="form-control" name="data_pesquisa" id="data_pesquisa" placeholder="Data de Pesquisa"/>
                              <span class="input-group-addon" style="border-radius: 0px!important;">
                                  <span class="glyphicon glyphicon-calendar"></span>
                              </span>
           </div>
        </div>
        
        <div class="col-md-3 col-sm-6 col-xs-12">
            <div class="form-group">
                      <div class="input-group" style='width:100%;'>
                        <button type="submit" class="btn btn-info" style="margin-top: 25px;" data-dismiss="modal" id="info-suporte">Pesquisar Data</button>
                      </div>
                  </div>
        </div>
        
        <div class="col-md-6 col-sm-6 col-xs-12">
            <div class="form-group">
                      <div class="input-group" style='width:100%;' id="date_info">
                      </div>
             </div>
        </div>
        
        </form>
        
        <!-- Texto de Selecção de Datas de Pesquisa -->
        
        
    </div>
 </div>

</div>

<div class="table-responsive">

<table id="example" class="display responsive nowrap table-striped dataTable" cellspacing="0" width="100%">
  <thead>
    <tr>
      <th>Acções/ID</th>
      <th>Estado</th>
      <th>Data</th>
      <th>Empresa</th>
      <th>Cliente</th>
      <th>Contacto</th>
      <th>Transfergest</th>
      <th>Observações</th>
      
      
    </tr>
  </thead>
  <tfoot>
    <tr>
      <th>Acções/ID</th>
      <th>Estado</th>
      <th>Data</th>
      <th>Empresa</th>
      <th>Cliente</th>
      <th>Contacto</th>
      <th>Transfergest</th>
      <th>Observações</th>
    </tr>
  </tfoot>
</table>

</div>


</div>

</div>
</div>



<!-- MOdelos das Popups -->

<?php require 'mymodals.php'; ?>







<!-- End Line Page -->


<script src="js/bootbox.min.js"></script>
<script src="js/momentjs/moment-with-locales.min.js"></script>
<script src="js/bootstrap-datetimepicker.min.js"></script>
<script src="js/jquery.dataTables.min.js"></script>
<script src="js/dataTables.bootstrap.min.js"></script>
<script src="js/dataTables.buttons.min.js"></script>
<script src="js/buttons.colVis.js"></script>
<script src="js/buttons.print.min.js"></script>
<script src="js/buttons.html5.min.js"></script>
<script src="js/pdfmake.min.js"></script>
<script src="js/vfs_fonts.js"></script>
<script src="js/jszip.min.js"></script>
<script src="js/dataTables.colReorder.min.js"></script>
<script src="js/select2.full.min.js"></script>
<script src="js/shopinfo.js"></script>





<script>
  setTimeout(function() {$(".back").fadeOut(); GetNIF(); GetEmpresa();});
</script>


<script>


var date = new Date();
d = date.setDate(date.getDate());
h = date.setHours(date.getHours());

// Data de Pesquisa de Clientes e de Suportes ao Sistema do seu backoofice
    $("#data_pes, #data_agenda_edit").datetimepicker({ ignoreReadonly: true, format: 'DD/MM/YYYY', 
    locale: 'pt',widgetPositioning: {vertical: 'bottom'}, defaultDate: h});


$(".search_results").css('display', 'none');

var table;
var editor;
var myElem;
var estado;

base64='';

nome = 'Osb Unipessoal, Lda';
nif = '123123123';
ravt = '1234';
morada = 'Estrada Sta. Eulalia';
cod_postal = '8200-000 Albufeira';
tel = '00351289150167';
tlm = '00351914564564';
email = 'suporte@oseubackoffice.com';
dominio = 'oseubackoffice.com';


footer_txt = nome+' | NIF:'+nif+' | RNAVT:'+ravt+'\n '+morada+' '+cod_postal+' | TEL:'+tel+' - TLM:'+tlm+'\n EMAIL:'+email+' | URL:'+dominio; 


$("#example, .search").css("display","none");
  
   
function selectEstado()  
{
     if ($("#estado_edit").val() == 'Resolvido')
        {
            $( "#data_agenda_edit" ).prop('disabled',true);
            $( "input[name='data_agenda']" ).prop('disabled',true);
            $( "#obs_suporte_edit" ).prop('disabled',true);
        }
        if ($("#estado_edit").val() == 'Nao Atende')
        {
            $( "#data_agenda_edit" ).prop('disabled',true);
            $( "input[name='data_agenda']" ).prop('disabled',true);
            $( "#obs_suporte_edit" ).prop('disabled',false);
            $("#estado_edit").prop('disabled',false);
        }
        if ($("#estado_edit").val() == 'Pendente')
        {
            $( "#data_agenda_edit" ).prop('disabled',false);
            $( "input[name='data_agenda']" ).prop('disabled',false);
            $( "#obs_suporte_edit" ).prop('disabled',false);
            $("#estado_edit").prop('disabled',false);
        }
        if ($("#estado_edit").val() == 'Remarcado')
        {
            $( "#data_agenda_edit" ).prop('disabled',false);
            $( "input[name='data_agenda']" ).prop('disabled',false);
            $( "#obs_suporte_edit" ).prop('disabled',false);
            $("#estado_edit").prop('disabled',false);
        }
}
        
  

function clearTable()
{
  table = $('#example').DataTable();
  table.destroy();

  $('#example').empty().html('<thead><tr><th>Acções/ID</th><th>Estado</th><th>Data</th><th>Empresa</th><th>Cliente</th><th>Contacto</th><th>Transfergest</th><th>Observações</th></tr></thead><tfoot><tr><th>Acções/ID</th><th>Estado</th><th>Data</th><th>Empresa</th><th>Cliente</th><th>Contacto</th><th>Transfergest</th><th>Observações</th></tr></tfoot>');
  $("#example_container").css('display','none');
}


function getFormattedDate(today) 
{
    var week = new Array('Domingo', 'Segunda-Feira', 'Terca-Feira', 'Quarta-Feira', 'Quinta-Feira', 'Sexta-Feira', 'Sabado');
    var day  = week[today.getDay()];
    var dd   = today.getDate();
    var mm   = today.getMonth()+1; //January is 0!
    var yyyy = today.getFullYear();

    if(dd<10)  { dd='0'+dd } 
    if(mm<10)  { mm='0'+mm } 

    return day+' '+dd+'/'+mm+'/'+yyyy;
}

// Aviso de Preencher os Campos 

function inputNoDataTable(){
  $('#Modalko .debug-url').html('Preencha um dos campos para efectuar a pesquisa!');
  $("#mensagem_ko").trigger('click');
  $('.back').fadeOut();
  $('.row .search, #example_wrapper, #example, .search_results').hide();  
}

function editAgendaTable(arr)
{
    
    var clientesModal = $('#agenda__modal');
    
    
        //Data do Cliente
        $("#data_ag_edit").val(moment(arr.proxima_data*1000).format("DD/MM/YYYY"));
        
        // Empresa
        $("#empresa_edit").val(arr.cliente);

        //Cliente
        

        $("#cliente_agenda_edit").val(arr.nome_pessoa);

        //Observacoes
        
        $("#obs_suporte_edit").val(arr.obs_suporte);
        
        //Telefone
        
        $("#telefone_edit").val(arr.telefone);
        
        //NIF
        
        $("#nif_agenda_edit").val(arr.nif);
        
        
        // Estado
        
        $("#estado_edit").html('<option value="'+arr.estado+'">'+arr.estado+'</option><option value="Resolvido">Resolvido</option><option value="Pendente">Pendente</option><option value="Remarcado">Remarcado</option><option value="Nao Atende">Nao Atende</option>');
        
        
        if ($("#estado_edit").val() == 'Resolvido')
        {
            $( "#data_agenda_edit" ).prop('disabled',true);
            $( "input[name='data_agenda']" ).prop('disabled',true);
            $("#estado_edit").prop('disabled',true);
            $( "#obs_suporte_edit" ).prop('disabled',true);
        }
        if ($("#estado_edit").val() == 'Nao Atende')
        {
            $( "#data_agenda_edit" ).prop('disabled',true);
            $( "input[name='data_agenda']" ).prop('disabled',true);
            $("#estado_edit").prop('disabled',false);
            $( "#obs_suporte_edit" ).prop('disabled',false);
        }
        if ($("#estado_edit").val() == 'Pendente')
        {
            $( "#data_agenda_edit" ).prop('disabled',false);
            $( "input[name='data_agenda']" ).prop('disabled',false);
            $("#estado_edit").prop('disabled',false);
            $( "#obs_suporte_edit" ).prop('disabled',false);
        }
        if ($("#estado_edit").val() == 'Remarcado')
        {
            $( "#data_agenda_edit" ).prop('disabled',false);
            $( "input[name='data_agenda']" ).prop('disabled',false);
            $("#estado_edit").prop('disabled',false);
            $( "#obs_suporte_edit" ).prop('disabled',false);
        }
       
        
        
        
        
        
    
    clientesModal.modal('show');
}


function visualizeAgendaTableByBugs(arr)
{
    var id = arr.id;
    var s = '';
    var BugsModal = $('#visualize_bugs_agenda');
    
    BugsModal.modal('show');
    
    
    dataValue='action=2&id='+id+"&data_agenda="+$("#data_pesquisa").val();
  //console.log(dataValue);
    $.ajax({ url:'ag/action_agenda.php',
    data:dataValue,
    type:'POST', 
    cache:false,
    success: function(data){
      $('.back').fadeOut();
      //console.log(data);

      arr = JSON.parse(data);
      
          if (arr == null || arr.length < 1)
          {
              BugsModal.modal('hide');
            $('.debug-url').html('Não existe valores dos bugs que este tipo de contribuinte pertencente a mesma data.');
            $("#Modalko").modal();
          }
          else 
          {
        
      
            for(i=0;i<arr.length;i++)
            {
                var str = arr[i].descricao;
                
                s += '<div class="col-xs-12" style="text-align: justify;"><h4>Bug Numero '+arr[i].id+'</h4></div><br><div class="col-xs-12" style="text-align: justify;"><strong>Link do Bug</strong></div><br><div class="col-xs-12" style="text-align: justify;">'+arr[i].link+'</div><br><div class="col-xs-12" style="text-align: justify;"><strong>Descrição do Bug</strong></div><br><div class="col-xs-12" style="text-align: justify;">'+str+'</div><br><div class="col-xs-12" style="text-align: justify;"><strong>Estado do Bug</strong></div><br><div class="col-xs-12" style="text-align: justify;">'+arr[i].estado+'</div><br><div class="col-xs-12" style="text-align: justify;"><br><p style="border: 1px solid #000;"></p></div><br>';
            }
            $("#bus_agenda").html(s);
          }
          
          
          
            
          
    },
    error:function(data){
        //console.log(data);
      }
    });
}


    // Guardar a edição da agenda de estados no sistema
    
    
    function saveIt(vl){
		dataValue="action=1&id="+vl+"&cliente="+$("#cliente_agenda_edit").val()+"&telefone="+$("#telefone_edit").val()+"&data_agenda="+$('#data_ag_edit').val()+"&obs_suporte="+$("#obs_suporte_edit").val()+"&estado="+$("#estado_edit").val();


	//console.log(dataValue);

$.ajax({ url:'ag/action_agenda.php',
    data:dataValue,
    type:'POST',
    cache:false,
    success: function(data){

//console.log(data);

     if (data == 1) {
     	$('.debug-url').html('O Numero da Agenda de Assistencia <strong class="cpt">'+vl+'</strong> foi modificado com sucesso');
          $("#mensagem_ok").trigger('click');
          $('html, body').animate({ scrollTop: 0 }, 500);
          setTimeout(function(){
          $('#Modalok').modal('hide');},2500);
      reWriteRowTable(vl);
     }
     else if (data == 0){
       $('.cancel-edit').trigger('click');
       $('.debug-url').html('Agenda #<strong>'+vl+'</strong> não foi modificada!');
          $("#Modalko").modal();
          setTimeout(function(){  $('#Modalko').modal('hide');}, 2500);
     }
},
    error:function(){
        $('.cancel-edit').trigger('click');
        $('.debug-url').html('Agenda # <strong> ' +vl+ ' </strong> não foi modificada, p.f. verifique a ligação Wi-Fi.');
        $("#Modalko").modal();

}
  });

}

	// Escrever os dados alterados na datatable

	function reWriteRowTable(vl){

		var clientesModal = $('#agenda__modal');

		table = $('#example').DataTable();

		data = table.row("#"+vl ).data();

    myDate=$('#data_ag_edit').val().split("/");
    var newDate=myDate[1]+"/"+myDate[0]+"/"+myDate[2];
    x = new Date(newDate).getTime()/1000;

    data['proxima_data'] = x;

    data['obs_suporte'] = $("#obs_suporte_edit").val();

    data['estado'] = $("#estado_edit").val();
    
    data['cliente'] = $("#cliente_agenda_edit").val();

    data['telefone'] = $("#telefone_edit").val();


		table.row("#"+vl ).data(data).draw(false);

		
		$("#"+vl).addClass('blink').removeClass('selected');
		setTimeout(function(){ $("#"+vl).removeClass('blink'); }, 10000);
    table.ajax.reload();

    clientesModal.modal('hide');
    
    

	}
    
    

    function getDayOfWeek(date) 
    {
      var dayOfWeek = new Date(date).getDay();    
      return isNaN(dayOfWeek) ? null : ['Domingo', 'Segunda-Feira', 'Terca-Feira', 'Quarta-feira', 'Quinta-Feira', 'Sexta-Feira', 'Sabado'][dayOfWeek];
    }
    
    
    // Pesquisar por Data em Datatables
    $("#search_data").submit(function(e)
    {
        e.preventDefault();
        
        clearTable();
        
        
        var data_pesquisa = $("#data_pesquisa").val();
        
        // Pesquisa por Datatables
        
        if (data_pesquisa == '')
        {
            inputNoDataTable();
        }
        else
        {
            var split_date = data_pesquisa.split("/");
            
            var monthNames = ["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outobro", "Novembro", "Dezembro"];
        
            var date_t = split_date[1] + "/" + split_date[0] + "/" + split_date[2];
            
            var date_s = split_date[2] + "-" + split_date[1] + "-" + split_date[0];
            
            var dia_semana = getDayOfWeek(date_s);
            
            var dia = split_date[0];
            
            
            var month_name = function(dt){
            mlist = [ "Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro" ];
              return mlist[dt.getMonth()];
            };
            
            
            month = month_name(new Date(date_t));
            

            
            var ano = split_date[2];
            
            dateString = dia_semana + "," + dia + " de " + month + " de " + ano;
        
            /*const parts = date_t.split('/');
            parts.unshift(parts.pop());
            parts[1] -= 1;
            const dateString = new Intl.DateTimeFormat('pt', {
              weekday: 'long',
              year: 'numeric',
              month: 'long',
              day: 'numeric',
              timeZone: 'UTC'
            }).format(Date.UTC(...parts)).replace(/,/g, '');*/
            $('#date_info').html('<input readonly style="border-radius: 4px; margin-top: 25px;" type="text" class="form-control" value="'+dateString+'"  placeholder="Tipo Servico">');
            
            
            searchValue('action=1&data_pesquisa='+data_pesquisa);
        }
        
        
    });
    
    
    // Edição de Valores nas Agendas de Suporte
    
  

    
    
    // Função de Criação de uma Datatable
    
    function searchValue(vl){
        $("#example, #example_container2, #example_container, .search").css("display","block");
        $(".search_results").css('display', 'block');

        console.log(vl);
        
        table = $('#example').DataTable( {
        dom: "Blfrtip",
        rowId: "id",
        "paging": true,
        "serverside":true,
        order: [],
        "drawCallback": function( settings ) {
        setTimeout(function(){
          
          info = table.page.info();
              $('.search_results').show();
              $("#Modalko").modal('hide');
              $('#nr_resultados').html("<span class='glyphicon glyphicon-tags'></span>&nbsp; Resultados: "+info.recordsTotal);
              $("#example, #example_container, #example_wrapper, .search").css("display","block");
              $(".search_results").css('display', 'block');
              
          
          
         }, 500);
        },
        "ajax": 
          {
            url : "action_datatables_agenda.php?"+vl,
            method: "GET"
          },
          columns: [
            { data: "id", render: function ( data, type, row ) {
              return '<button title="Editar Serviço:'+data+'" class="btn-sm btn btn-info btn-edit"><span class="glyphicon glyphicon-edit"></span> '+data+'</button><button title="Ver Bug'+data+'" class="btn-sm btn btn-info btn-vision"> B </button>';
              }},
              { data: "estado" },
            { data: "proxima_data" , render: function ( data, type, row ) {
              myDate = new Date(data*1000);
              a = moment(myDate).format('DD/MM/YYYY');
              return a;
              }
            },
            { data: "cliente" },
            { data: "nome_pessoa", render: function ( data, type, row ) {
            if (data == "")
            {
                a = "-/-";
            }
            else
            {
                a = data;
            }
            return a;
        } },
            { data: "telefone", render: function ( data, type, row ) {
            if (data == 0)
            {
                a = "-/-";
            }
            else
            {
                a = data;
            }
            return a;
        } },
             { data: "nif", render: function ( data, type, row ) {
                   return '<a href="http://'+data+'.transfergest.com" target="_blank">'+data+'.transfergest.com</a>';
            }
            },
            { data: "obs_suporte", render: function ( data, type, row ) {
            if (data == "")
            {
                a = "-/-";
            }
            else
            {
                a = data;
            }
            return a;
        } }
           
            
            
          ],
          select: 
            {
              style:    'os',
              selector: 'td:first-child'
            },
        buttons: [
          {
            extend: 'colvis', text: '<i class="fa fa-scissors"></i>',
            titleAttr: 'Esconder Colunas',
            exportOptions: { orthogonal: 'export', columns: ':visible' }
          },
          {
            extend: 'print', 
            text: '<i class="fa fa-print"></i>',
            titleAttr: 'Imprimir',
            exportOptions: { orthogonal: 'export', columns: ':visible' }
          },
          {
            extend: 'excelHtml5',
            text: '<i class="fa fa-file-excel-o"></i>',
            titleAttr: 'Excel',
            key: {key: 't', altkey: true},
            exportOptions: { orthogonal: 'export', columns: ':visible' },
            action:function(e,dt,button,config)
            {
              var date_pes = $("#data_pesquisa").val()+"_Suporte de Assistencia Tecnica";
                
                
              config.filename = 'agenda ' + "_ " + date_pes;
              $.fn.dataTable.ext.buttons.excelHtml5.action(e,dt,button,config);
            }
          },
          {  
            extend: '',
            className:'buttons-pdf-format',
            text: '<i class="fa fa-file-pdf-o"></i>',
            titleAttr: 'PDF'
           },
           {
            extend: 'pdfHtml5',
            className:'a4landscape hidden',
            text: '<i class="fa fa-file-pdf-o"></i>',
            orientation: 'landscape',
            pageSize: 'A4',
            footer: true,
            exportOptions: { orthogonal: 'export', columns: ':visible' },
            action:function(e,dt,button,config){
              var date_pes = $("#data_pesquisa").val()+"_Suporte de Assistencia Tecnica";
              
                
              config.filename = 'agenda ' + "_ " + date_pes;
              
              
              config.title = 'Agenda: ' + $("#data_pesquisa").val() + " - Suporte de Assistencia Técnica";
              
              $.fn.dataTable.ext.buttons.pdfHtml5.action(e,dt,button,config);
            },
              customize: function(doc) {

/*MARGENS DOCUMENTO*/
                doc.pageMargins = [5, 5, 5, 40];

/*AJUSTE DAS COLUNAS À PÁGINA*/
                doc.content[1].table.widths = Array(doc.content[1].table.body[0].length + 1).join('*').split('');

/* LOGO */
                doc.content.splice( 1, 0, {
                   // margin: [left, top, right, bottom]
                   margin: [ 10, 0, 0, 12 ],
                   alignment: 'center',
                   width:200,
                   image: 'data:image/png;base64,'+base64
                });

/* RESULTADOS PESQUISA */
                /*doc.content.splice( 2, 0, {
                   margin: [ 10, -5, 0, 5],
                   alignment: 'right',
                   fontSize: '11',
                   color : 'black',
                   text:'Pesquisa ( '+title+' )'
                 });*/

/* DADOS DA EMPRESA */
                doc['footer'] = {text: footer_txt, fontSize: '8', margin : [-10,0,0,10], color : 'black', alignment: 'center' };

             }    
          },
          {
            extend: 'pdfHtml5',
            className:'a4portrait hidden',
            text: '<i class="fa fa-file-pdf-o"></i>',
            orientation: 'portrait',
            pageSize: 'A4',
            footer: true,
            exportOptions: { orthogonal: 'export', columns: ':visible' }, action:function(e,dt,button,config){
              var date_pes = $("#data_pesquisa").val()+"_Suporte de Assistencia Tecnica";
                
                
              config.filename = 'agenda ' + "_ " + date_pes;
              config.title = 'Agenda: ' + $("#data_pesquisa").val() + " - Suporte de Assistencia Técnica";
              
              $.fn.dataTable.ext.buttons.pdfHtml5.action(e,dt,button,config);
            },
            customize: function(doc) {

/*MARGENS DOCUMENTO*/
                doc.pageMargins = [5, 5, 5, 40];

/*AJUSTE DAS COLUNAS À PÁGINA*/
                doc.content[1].table.widths = Array(doc.content[1].table.body[0].length + 1).join('*').split('');

/* LOGO */
                doc.content.splice( 1, 0, {
                   // margin: [left, top, right, bottom]
                   margin: [ 10, 0, 0, 12 ],
                   alignment: 'center',
                   width:200,
                   image: 'data:image/png;base64,'+base64
                });

/* RESULTADOS PESQUISA */
                /*doc.content.splice( 2, 0, {
                   margin: [ 10, -5, 0, 5],
                   alignment: 'right',
                   fontSize: '11',
                   color : 'black',
                   text:'Pesquisa ( '+title+' )'
                 });*/

/* DADOS DA EMPRESA */
                doc['footer'] = {text: footer_txt, fontSize: '8', margin : [-10,0,0,10], color : 'black', alignment: 'center' };

             }    
          },
          {
            extend: 'pdfHtml5',
            className:'a3portrait hidden',
            text: '<i class="fa fa-file-pdf-o"></i>',
            orientation: 'portrait',
            pageSize: 'A3',
            footer: true,
            exportOptions: { orthogonal: 'export', columns: ':visible' },
            action:function(e,dt,button,config){
                var date_pes = $("#data_pesquisa").val()+"_Suporte de Assistencia Tecnica";
                
                
              config.filename = 'agenda ' + "_ " + date_pes;
              config.title = 'Agenda: ' + $("#data_pesquisa").val() + " - Suporte de Assistencia Técnica";
              config.pagesize = $('#pagesize').val();
              config.orientation = $('#orientation').val();
              $.fn.dataTable.ext.buttons.pdfHtml5.action(e,dt,button,config);
            },
            customize: function(doc) {

/*MARGENS DOCUMENTO*/
                doc.pageMargins = [5, 5, 5, 40];

/*AJUSTE DAS COLUNAS À PÁGINA*/
                doc.content[1].table.widths = Array(doc.content[1].table.body[0].length + 1).join('*').split('');

/* LOGO */
                doc.content.splice( 1, 0, {
                   // margin: [left, top, right, bottom]
                   margin: [ 10, 0, 0, 12 ],
                   alignment: 'center',
                   width:200,
                   image: 'data:image/png;base64,'+base64
                });

/* RESULTADOS PESQUISA */
                /*doc.content.splice( 2, 0, {
                   margin: [ 10, -5, 0, 5],
                   alignment: 'right',
                   fontSize: '11',
                   color : 'black',
                   text:'Pesquisa ( '+title+' )'
                 });*/

/* DADOS DA EMPRESA */
                doc['footer'] = {text: footer_txt, fontSize: '8', margin : [-10,0,0,10], color : 'black', alignment: 'center' };

             }    
          },
          {
            extend: 'pdfHtml5',
            className:'a3landscape hidden',
            text: '<i class="fa fa-file-pdf-o"></i>',
            orientation: 'landscape',
            pageSize: 'A3',
            footer: true,
            exportOptions: { orthogonal: 'export', columns: ':visible' },
            action:function(e,dt,button,config){
              var date_pes = $("#data_pesquisa").val()+"_Suporte de Assistencia Tecnica";
                
                
              config.filename = 'agenda ' + "_ " + date_pes;
              config.title = 'Agenda: ' + $("#data_pesquisa").val() + " - Suporte de Assistencia Técnica";
              config.pagesize = $('#pagesize').val();
              config.orientation = $('#orientation').val();
              $.fn.dataTable.ext.buttons.pdfHtml5.action(e,dt,button,config);
            },
            customize: function(doc) {

/*MARGENS DOCUMENTO*/
                doc.pageMargins = [5, 5, 5, 40];

/*AJUSTE DAS COLUNAS À PÁGINA*/
                doc.content[1].table.widths = Array(doc.content[1].table.body[0].length + 1).join('*').split('');

/* LOGO */
                doc.content.splice( 1, 0, {
                   // margin: [left, top, right, bottom]
                   margin: [ 10, 0, 0, 12 ],
                   alignment: 'center',
                   width:200,
                   image: 'data:image/png;base64,'+base64
                });

/* RESULTADOS PESQUISA */
                /*doc.content.splice( 2, 0, {
                   margin: [ 10, -5, 0, 5],
                   alignment: 'right',
                   fontSize: '11',
                   color : 'black',
                   text:'Pesquisa ( '+title+' )'
                 });*/

/* DADOS DA EMPRESA */
                doc['footer'] = {text: footer_txt, fontSize: '8', margin : [-10,0,0,10], color : 'black', alignment: 'center' };

             }
          },


          {
            extend: 'pdfHtml5',
            className:'a2portrait hidden',
            text: '<i class="fa fa-file-pdf-o"></i>',
            orientation: 'portrait',
            pageSize: 'A2',
            footer: true,
            exportOptions: { orthogonal: 'export', columns: ':visible' },
            action:function(e,dt,button,config){
              var date_pes = $("#data_pesquisa").val()+"_Suporte de Assistencia Tecnica";
                
                
              config.filename = 'agenda ' + "_ " + date_pes;
              config.title = 'Agenda: ' + $("#data_pesquisa").val() + " - Suporte de Assistencia Técnica";

              
              $.fn.dataTable.ext.buttons.pdfHtml5.action(e,dt,button,config);
            },
            customize: function(doc) {

/*MARGENS DOCUMENTO*/
                doc.pageMargins = [5, 5, 5, 40];

/*AJUSTE DAS COLUNAS À PÁGINA*/
                doc.content[1].table.widths = Array(doc.content[1].table.body[0].length + 1).join('*').split('');

/* LOGO */
                doc.content.splice( 1, 0, {
                   // margin: [left, top, right, bottom]
                   margin: [ 10, 0, 0, 12 ],
                   alignment: 'center',
                   width:200,
                   image: 'data:image/png;base64,'+base64
                });

/* RESULTADOS PESQUISA */
                /*doc.content.splice( 2, 0, {
                   margin: [ 10, -5, 0, 5],
                   alignment: 'right',
                   fontSize: '11',
                   color : 'black',
                   text:'Pesquisa ( '+title+' )'
                 });*/

/* DADOS DA EMPRESA */
                doc['footer'] = {text: footer_txt, fontSize: '8', margin : [-10,0,0,10], color : 'black', alignment: 'center' };

             }   
          },
          {
            extend: 'pdfHtml5',
            className:'a2landscape hidden',
            text: '<i class="fa fa-file-pdf-o"></i>',
            orientation: 'landscape',
            pageSize: 'A2',
            footer: true,
            exportOptions: { orthogonal: 'export', columns: ':visible' },
            action:function(e,dt,button,config){
              var date_pes = $("#data_pesquisa").val()+"_Suporte de Assistencia Tecnica";
                
                
              config.filename = 'agenda ' + "_ " + date_pes;
              config.title = 'Agenda: ' + $("#data_pesquisa").val() + " - Suporte de Assistencia Técnica";
              $.fn.dataTable.ext.buttons.pdfHtml5.action(e,dt,button,config);
            },
            customize: function(doc) {

/*MARGENS DOCUMENTO*/
                doc.pageMargins = [5, 5, 5, 40];

/*AJUSTE DAS COLUNAS À PÁGINA*/
                doc.content[1].table.widths = Array(doc.content[1].table.body[0].length + 1).join('*').split('');

/* LOGO */
                doc.content.splice( 1, 0, {
                   // margin: [left, top, right, bottom]
                   margin: [ 10, 0, 0, 12 ],
                   alignment: 'center',
                   width:200,
                   image: 'data:image/png;base64,'+base64
                });

/* RESULTADOS PESQUISA */
                /*doc.content.splice( 2, 0, {
                   margin: [ 10, -5, 0, 5],
                   alignment: 'right',
                   fontSize: '11',
                   color : 'black',
                   text:'Pesquisa ( '+title+' )'
                 });*/

/* DADOS DA EMPRESA */
                doc['footer'] = {text: footer_txt, fontSize: '8', margin : [-10,0,0,10], color : 'black', alignment: 'center' };

             }
          },

          {
           extend: 'csv',
           filename: 'agenda',
           text: '<i class="fa fa-file-text-o"></i>',
           titleAttr: 'CSV',
           key: {key: 'l', altkey: true},
           exportOptions: { orthogonal: 'export', columns: ':visible' },
            action:function(e,dt,button,config){
              var date_pes = $("#data_pesquisa").val()+"_Suporte de Assistencia Tecnica";
                
                
              config.filename = 'agenda ' + "_ " + date_pes;
              config.title = 'Agenda:' + $("#data_pesquisa").val() + " - Suporte de Assistencia Técnica";
              $.fn.dataTable.ext.buttons.csvHtml5.action(e,dt,button,config);
            }
          }

          ],
              "language": {
                "emptyTable":"Sem resultados",         
                "paginate": {
                  "first":      "Primeiro",
                  "last":       "Ultimo",
                  "next":       "Seguinte",
                  "previous":   "Anterior"
                },
                "zeroRecords": "Não tem registos",
                "loadingRecords": "A carregar...",
                "processing":     "A processar ...",
                "info": "Página nº _PAGE_ de _PAGES_",
                "infoEmpty": "Sem registos disponiveis",
                "infoFiltered": "(filtro de _MAX_ registos totais)"
              },
              "createdRow": function ( row, data, index ) {
                $(row).attr('id', data[0]);
                t= parseInt(data["px"])+parseInt(data["cr"])+parseInt(data["bebe"]);

                 switch(data['estado']){
                   case "Resolvido":
                          $(row).addClass('w3-pale-green');break;
                   case "Pendente":
                          $(row).addClass('w3-pale-yellow');break;
                   case "Remarcado":
                          $(row).addClass('w3-pale-blue');break;
                   case "Nao Atende":
                          $(row).addClass('w3-pale-red');break;
                 }

              },


     });
     
     
       $('#example tbody').on( 'click', '.btn-edit', function () {
       arr = table.row($(this).parents('tr')).data();
       $('.servico_id').html(arr.id);
        editAgendaTable(arr);
    });
    
    
     $('#example tbody').on( 'click', '.btn-vision', function () {
       arr = table.row($(this).parents('tr')).data();
       $('.servico_id').html(arr.id);
        visualizeAgendaTableByBugs(arr);
    });

  $('#example tbody').on( 'click', '.btn-danger', function () {
       arr = table.row($(this).parents('tr')).data();
       confirmDeleteAssistenceClients(arr.id);
  });


  


$('#example_filter').addClass('col-xs-12 col-md-6 col-sm-12 pull-right').removeClass('dataTables_filter');
$('#example_length').addClass('col-xs-6 col-md-3 col-sm-4 pull-left').removeClass('dataTables_length');
$('.dt-buttons').addClass('col-xs-6 col-md-3 col-sm-4 pull-right').removeClass('dt-buttons');

$('#example_filter input').attr('placeholder','Nome ou NIF');

$('#example_wrapper').on( 'click', '.buttons-pdf-format', function () {
$('#pdf_conf').modal();
});


  }


  function tagDraw(){              //draw a request on add or remove tag
    var v= $(".input-sm").val();
    table.search(v).draw();
}

function generatePdf(p,o){
$('#pdf_conf').modal('hide');
$('.back').fadeIn();
setTimeout(function(){
$('.'+p+''+o).trigger('click');}, 500);
setTimeout(function(){$('.back').fadeOut();}, 1000);
} 
    





</script>


</body>
