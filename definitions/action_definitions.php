...
// Funcao de Retorno dos campos pretnecentes a tabela 'clientes_assistencia' na base de dados gestão de assistencia tecnica

function clientesDeAssistencia($conn)
{
	$obter_comp=" SELECT * FROM clientes_assistencia";
...
case '33':
$obter_comp=" SELECT nif FROM clientes_assistencia ORDER BY nif ASC";
$result = mysqli_query($conn, $obter_comp);
while($obj = mysqli_fetch_object($result)) {
$var[] = $obj;}
echo json_encode($var);
...
case '34':
$obter_comp=" SELECT cliente FROM clientes_assistencia ORDER BY cliente ASC";
...
$var = clientesDeAssistencia($conn);

// Resultado de obter so um campo da tabela 'clientes' que são convertidos numa listagem de arrays pertencentes a este filtro que é neste caso o NIF

$array = json_decode(json_encode($var), true);

$a = array ('client' => $array);

foreach($a['client'] as $result) {
	$i = $result['id'];
    $b[] = array('nif_emp' => $result['nif'], 'cliente' => $result['cliente']);
}

// Array geral dos filtros pertencentes a assistencia técnica


/*echo 'Total de Clientes de Assistencia - ';

'<pre>'.
print_r($array).
'</pre>';
*/
// Depois do Calculo do array (convertido em objecto através da função mysqli_fetch_object e depois a descoficaçação dos valores dos objectos mostrados na tabela de assistencia técnica) dos filtros pertencentes a assistencia técnica

/*echo 'Listagem de empresas dos clientes - ';
'<pre>'.
print_r($b).
'</pre>';
*/
...
// No Javascript convertes os dados num array de dados em json se quer mostrar so um valor vai
// buscar o valor de retorno da variavel que foi alterado no servidor (PHP) - arr[i].nif_emp
// Se For só com um valor mostra o 1º element do array da variavel que foi calculado no servidor - arr[0].nif_emp
